...
Note that LAVA eQTL analysis is still a beta feature, and implementation may change in future.

### Loading input

When using input pre-formatted for LAVA, which will be made available via the [LAVA website](https://ctg.cncr.nl/software/lava), adding the eQTL summary statistics is done as follows, where the root folder of the eQTL summary statistics is assumed to be `data/eqtl`:
...
The process.eqtl.input() function will first load the SNP-to-gene annotation for all the specified chromosomes into `input`, and set.tissue() subsequently loads the eQTL summary statistics for the specified tissue. Calling set.tissue() again with a different tissue will replace the summary statistics loaded for the current tissue. The set.chromosomes() function can also be used to load a different set of chromosomes, and in practice it may be practical to load only a single chromosome at a time to reduce the memory and loading time.

The eQTL summary statistics can also be provided in general data files, which should have the same format as the input files for regular phenotypes, plus an additional **GENE** column indicating which gene the SNP associations on each line belong to. Input files must be provided separately for each chromosome, with file names that are identical except for the chromosome code. Data is loaded as follows:

```{r, eval=F}
process.eqtl.text.input(input, "data/eqtl_text/whole_blood.chr[CHR].stats", chromosomes=14, sample.size=750)
```

The second argument of process.eqtl.text.input() specifies the eQTL file format, with the `[CHR]` a placeholder for the chromosome code; in this case, LAVA will look for the file `data/eqtl_text/whole_blood.chr14.stats`. The sample.size argument is optional, and can be omitted if a sample size column is present in the input files.
