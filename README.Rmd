...
As you can see in the input\$info data frame, additional columns have been added to indicate whether phenotypes are binary, and what the the proportion of cases are for binary phenotypes (computed from the cases / controls columns). (NOTE: the N column here is not used for the analysis).
...
To perform a bivariate *r*~g~ analysis, testing the local *r*~g~ between pairs of phenotypes, we simply pass the locus object the **run.bivar()** function

```{r}
run.bivar(locus)
```

When multiple phenotypes are entered simultaneously, the last phenotype will be treated as the target phenotype, and separate bivariate tests will be performed between this phenotype and all others. (Note that this test is symmetric, the order of the phenotypes doesn't matter!). Since a simulation procedure is used to obtain the p-values and confidence intervals, when running the tutorial your output will differ slightly from that shown here (this applies to all tests except the univariate ones).
...
The output also provides the full model *r*^2^, which quantifies the joint association of all predictor phenotypes with the outcome. As shown here, although the collinearity between asthma and diabetes hides their individual association, the model as a whole is still highly predictive of hypothyroidism.
