...
To analyse the bivariate test automatically conditional on univariate signal, you can also use the run.univ.bivar() function to perform both tests in one go
```{r}
...
If you are interested in e.g. the local bivariate genetic correlations between a large amount of phenotypes, we advice using e.g. a cluster computer. The example analysis script below shows how you might set up an R script that can be called from the command line
```{r, eval=F}
